 ...
 * $optionValues — Array of option value objects with swatch_color, swatch_image, display_label, price, etc.
 */
$step         = $step ?? new \stdClass();
$optionValues = $optionValues ?? [];
?>
<div class="gb-swatch-grid uk-flex uk-flex-wrap">
    <?php foreach ($optionValues as $val) :
        $label   = htmlspecialchars($val->display_label ?? $val->name ?? '', ENT_QUOTES, 'UTF-8');
        $valueId = (int) ($val->id ?? 0);
        $color   = htmlspecialchars($val->swatch_color ?? '#cccccc', ENT_QUOTES, 'UTF-8');
    ...
    <div class="gb-swatch"
        ...
        <div class="gb-swatch-circle"
             style="background-image:url('<?php echo htmlspecialchars($val->swatch_image, ENT_QUOTES, 'UTF-8'); ?>');background-size:cover;"></div>
        <?php else : ?>
        <div class="gb-swatch-circle" style="background-color:<?php echo $color; ?>;"></div>
        <?php endif; ?>

        <div class="gb-swatch-label"><?php echo $label; ?></div>

        <?php if ($priceLabel !== '') : ?>
        <div class="gb-swatch-price uk-text-small"><?php echo htmlspecialchars($priceLabel, ENT_QUOTES, 'UTF-8'); ?></div>
        <?php endif; ?>
