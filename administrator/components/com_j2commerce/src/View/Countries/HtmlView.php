...
namespace J2Commerce\Component\J2commerce\Administrator\View\Countries;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        if ($canDo->get('core.delete') && ($this->state->get('filter.enabled') == -2)) {
            $toolbar->delete('countries.delete', 'JTOOLBAR_DELETE_FROM_TRASH')->listCheck(true);
        }

        $toolbar->linkButton('zones')
            ->text('COM_J2COMMERCE_VIEW_ZONES')
            ->url('index.php?option=com_j2commerce&view=zones')
            ->icon('icon-location');
